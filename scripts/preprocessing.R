...
missing_rep <- model_overview_with_rep %>% filter(Env == rep) %>%
  pull(Env) %>% as.vector() # 29
missing_rep[which(missing_rep %in% caught)]

informative_env <- model_overview_with_rep %>% filter(Env != rep) %>% 
  mutate(repetability = as.numeric(repetability)) %>% arrange(repetability) %>%
  filter(repetability >= 0.3) # 28 removed
...
BLUEs <- BLUEs[, c("Env", "Hybrid", "Yield_Mg_ha")]
BLUEs_fil <- BLUEs %>% filter(Env %in% informative_env$Env) %>%
  mutate(Loc = gsub("(\\S+)\\_\\S+", "\\1", Env, perl = T),
         Year = gsub("\\S+\\_(\\S+)", "\\1", Env, perl = T)) %>%
  relocate(Loc, Year, .after = "Env")

write.table(BLUEs_fil, sprintf("%s/pheno_processed.txt", processed_data), row.names = F) # missing vales for genotypes in env like "ARH1_2016" exist since the raw data had missing data
...
marker_names <- read.table(sprintf("%s/5_Genotype_Data_All_Years.miss.1.mac.1.012.pos", processed_data), 
                           header = F, col.names = c("chr", "pos")) %>%
  mutate(variant = sprintf("C%sP%s", chr, pos))# unfiltered - 437214
sample_names <- read.table(sprintf("%s/5_Genotype_Data_All_Years.miss.1.mac.1.012.indv", processed_data), 
                           header = F, col.names = "Hybrid")

# check overlap between genotypic and phenotypic data

overlap_check_geno_pheno <- trait_data %>% distinct(Hybrid) %>% 
  mutate(present = ifelse(Hybrid %in% sample_names$Hybrid, TRUE, FALSE))
...
#     Var1   Freq
#       0   49642  # the one to start working. I will initially upload these markers, filtered with minor allele count of 1.
#  (0,0.1] 359131 # here onwards some sort of imputation will be needed. 
#(0.1,0.2]  18679 
#(0.2,0.3]   5166
#(0.3,0.4]   2385
#(0.4,0.5]   1142
...
geno_data <- data.table::fread(sprintf("%s/5_Genotype_Data_All_Years.miss.1.mac.1.012", processed_data), header = F) # less in number since only biallilic ones were output
geno_data_df <- cbind(sample_names$Hybrid, setDF(geno_data[, 2:ncol(geno_data)]))
colnames(geno_data_df) <- c("Hybrid", marker_names$variant)
data.table::fwrite(geno_data_df, sprintf("%s/geno_processed.miss.1.mac.1.biallelic.txt", processed_data)) # missing vales for genotypes in env like "ARH1_2016" exist since the raw data had missing data
system(sprintf(sprintf("xz %s/geno_processed.miss.1.mac.1.biallelic.txt", processed_data))) # use xz to decompress 
...
overlap_check_env_pheno %>% count(present) # looks good, may be standardized for the time being, i will upload the raw data
...
overlap_check_soil_pheno %>% count(present) # looks good, may be standardized for the time being, i will upload the raw data
...
overlap_check_EC_pheno %>% count(present) # looks good, may be standardized for the time being, i will upload the raw data
...
overlap_check_meta_pheno %>% count(present) # all present. # looks good, may be standardized for the time being, i will upload the raw data
